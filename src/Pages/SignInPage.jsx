import React from "react";
import { HiOutlineMail } from "react-icons/hi";
import { HiOutlineLockClosed } from "react-icons/hi";
import {NavLink,} from 'react-router-dom'
import { GoogleButton } from 'react-google-button';



function SignInPage() {
  const handleGoogleLogin = () => {
    // Use the Google Sign-In API to sign the user in
  };

  return (
    
    <div className="min-h-screen bg-loginimg bg-cover bg-center bg-no-repeat" id="bg-image"
    
  
>
  <div className="bg-[#2D1707] opacity-70  text-white pt-7">
    

<div >
      <div className="sm:mx-auto opacity-100 sm:w-full sm:max-w-md"
      
      >
        <h2 className="text-center text-[#D4C69D] text-3xl font-extrabold">
          Log in 
        </h2>
        {/* <p className="mt-2 text-center text-sm text-gray-600 max-w">
          Or{" "}
          <a href="#" className="font-medium text-indigo-600 hover:text-indigo-500">
            start your 14-day free trial
          </a>
        </p> */}
      </div>
</div>
      <div className="pb-7 mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className=" bg-transparent hover: py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-[#D4C69D] rounded-md">
          <form className="space-y-6 bg-transparent hover:shadow-md " action="#" method="GET">
            <div>
              <label htmlFor="name" className=" block text-sm font-medium text-[#D4C69D]">
                Username or Email
              </label>
              <div className="relative">
  <span className="absolute inset-y-0 left-0 flex items-center pl-2">
    <HiOutlineMail className="h-5 w-5 text-gray-400" />
  </span>
  <input
    id="name"
    name="name"
    type="text"
    autoComplete="name"
    required
    className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md bg-white shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm pl-10 bg-transparent text-[#2D1707]"
    placeholder="Username"
  />
</div>

            </div>

          

            <div className="opacity-1">
              <label htmlFor="password" className="block text-sm font-medium opacity-100 text-[#D4C69D]">
                Password
              </label>
              <div className="relative">
  <span className="absolute inset-y-0 left-0 flex items-center pl-2">
    <HiOutlineLockClosed className="h-5 w-5 text-gray-400" />
  </span>
  <input
    id="password"
    name="password"
    type="password"
    autoComplete="current-password"
    required
    className="appearance-none block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none text-[#2D1707] sm:text-sm pl-10 bg-transparent"
    placeholder="Password"
  />
</div>

            </div>

            <div className="opacity-100 text-[#2D1707] hover:scale-x-95 ">
              <button
                type="submit"
                className="w-full flex justify-center py-2 px-4 font-bold border border-transparent rounded-md   scale-100  text-[#2D1707] bg-[#D4C69D] hover:bg-white  shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 "
              >
                Log in
              </button>
            </div>
            <div className="font-bold pt-0 hover:text-white scale-75 hover:underline text-2xl text-center text-[#D4C69D]"><NavLink to={'/forgot password'}>Forgot Password?</NavLink></div>
            <div class="flex space-x-4 items-center hover:underline-offset-4">
            <GoogleButton onClick={handleGoogleLogin} />
</div>
<div class="flex space-x-4 items-center">
  <button
    type="submit"
    class="w-full flex  py-2 px-4 border border-transparent rounded-md drop-shadow-md text-sm  font-bold text-[#2D1707] bg-white hover:bg-[#D4C69D] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 btn-with-icon"
  >
    
    <img src="data:image/jpeg;base64," alt="Google icon" class="h-6 w-6 ml-0 px-0 space-x-0 mr-2 icon-left " />
    <h1 className="mx-6 ">Continue with Facebook</h1>
    
  </button>
</div>
 
          </form>
          <div className="pt-7 ">
            <h1 className="pl-7 font-bold text-xl">No account yet? <span className="text-[#D4C69D] font-bold hover:underline text-xl"><NavLink to={'/register'}>Register here!!</NavLink></span></h1>
          </div>
        </div>
      </div>
      
      {/* <div>
        <img src="https://www.newtimes.co.rw/thenewtimes/uploads/images/2022/12/13/thumbs/1200x700/8193.jpg" className="w-full"/>
      </div> */}
    </div>
    </div>
      
  );
}

export default SignInPage;